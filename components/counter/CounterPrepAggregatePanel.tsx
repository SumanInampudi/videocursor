...
import {
  buildCounterPrepAggregate,
  type CounterPrepAggregateInputOrder,
} from "@/lib/counter-kds";
import { orderTicketLabel } from "@/lib/order-channel";
import type { OrderChannel } from "@prisma/client";

type CounterPrepOrder = CounterPrepAggregateInputOrder & {
...
type CounterPrepAggregatePanelProps = {
  orders: CounterPrepOrder[];
};

export function CounterPrepAggregatePanel({ orders }: CounterPrepAggregatePanelProps) {
  const rows = useMemo(() => buildCounterPrepAggregate(orders), [orders]);
  ...
  if (rows.length === 0) {
    return (
      <section className="rounded-lg border border-gray-200 bg-white px-4 py-3 shadow-sm">
        ...
        <p className="mt-1 text-xs text-gray-500">No pending retail items in New or Picking.</p>
      </section>
    );
  }
  ...
  return (
    <section className="rounded-lg border border-teal-200/60 bg-white shadow-sm">
      <div className="flex items-center justify-between gap-3 border-b border-gray-100 px-4 py-2.5">
        <div>
          ...
          <p className="text-[11px] text-gray-500">
            Totals across New + Picking · pending qty only · tap a row for tickets
          </p>
        </div>
        <span className="shrink-0 rounded-full bg-teal-700 px-2.5 py-1 text-xs font-bold tabular-nums text-white">
